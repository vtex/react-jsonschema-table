...
class LinkSearch extends React.Component {
    ...
    if (this.state.searchValue && this.state.searchResultValue) {
      ...
      if (searchResults.length === 0) {
        searchResults.push(
          <div>
            ...
            <button
              className={`h-25 br3 bg-blue b--blue white w-100 dim ${this.state.focusedIndex === 0 ? 'shadow-1' : ''}`}
      ...
      searchResultDiv = (
        <div>
          ...
          <h4>
            {`Resultados${this.state.totalRows > this.state.searchResultValue.length ? ` (Mostrando 10 de${this.state.totalRows})` : ''}`}
          </h4>
          {searchResults}
        </div>
      )
    ...
    return (
      <HotKeys handlers={handlers}>
        ...
        <input
          ...
          placeholder={`Buscar um registro para associar por '${this.props.linkedField}'`}
  ...
  onMakeSearch = () => {
    ...
    this.timeout = null
    // Do the search
    var filter = `_where=${this.props.linkedField}=*${this.state.searchValue}*`

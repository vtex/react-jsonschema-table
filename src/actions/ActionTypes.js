export const ITEMS_LOAD_BEGAN = 'ITEMS_LOAD_BEGAN'
export const ITEMS_LOAD_SUCCESS = 'ITEMS_LOAD_SUCCESS'
export const ITEMS_LOAD_FAIL = 'ITEMS_LOAD_FAIL'
export const DELETE_ITEM_BEGAN = 'DELETE_ITEM'
export const DELETE_ITEM_SUCCESS = 'DELETE_ITEM'
export const DELETE_ITEM_FAIL = 'DELETE_ITEM'
export const SAVE_ITEMS_CHANGES_BEGAN = 'SAVE_ITEMS_CHANGES'
export const SAVE_ITEMS_CHANGES_SUCCESS = 'SAVE_ITEMS_CHANGES'
export const SAVE_ITEMS_CHANGES_FAIL = 'SAVE_ITEMS_CHANGES'
export const REMOVE_ITEM = 'REMOVE_ITEM'
export const ADD_ITEM = 'ADD_ITEM'
export const EXPORT_CHECKED_ITEMS = 'EXPORT_CHECKED_ITEMS'
export const SELECT_CELL = 'SELECT_CELL'
export const EDIT_CELL = 'EDIT_CELL'
export const EXIT_EDIT_CELL = 'EXIT_EDIT_CELL'
export const CLEAN_SELECTION = 'CLEAN_SELECTION'
export const SELECT_FILLHANDLE_CELL = 'SELECT_FILLHANDLE_CELL'
export const CLEAN_MASS_SELECTION = 'CLEAN_MASS_SELECTION'
export const SELECT_CELLS_RANGE = 'SELECT_CELLS_RANGE'
export const ITEM_STAGING = 'ITEM_STAGING'
export const MARK_ITEM_TO_REMOVE = 'MARK_ITEM_TO_REMOVE'
export const CHECK_ITEM = 'CHECK_ITEM'
export const CANCEL_STAGING = 'CANCEL_STAGING'
export const OPEN_DOCUMENT_IN_FORM = 'OPEN_DOCUMENT_IN_FORM'
export const DELETE_CHECKED_ITEMS = 'DELETE_CHECKED_ITEMS'
export const CHANGE_COLUMN_VISIBILITY = 'CHANGE_COLUMN_VISIBILITY'
export const VIEW_ALL_COLUMNS = 'VIEW_ALL_COLUMNS'
export const CHANGE_ROW_STATUS = 'CHANGE_ROW_STATUS'
export const PASTE_DATA = 'PASTE_DATA'
export const COPY_FROM_SELECTED_RANGE = 'COPY_FROM_SELECTED_RANGE'
export const UNDO_CHANGE = 'UNDO_CHANGE'
export const REDO_CHANGE = 'REDO_CHANGE'
export const CHANGE_CHECKED_FILTER = 'CHECKED_FILTER_CHANGE'
export const CHANGE_STAGING_FILTER = 'CHANGE_STAGING_FILTER'
export const CHANGE_INVALID_ITEMS_FILTER = 'CHANGE_INVALID_ITEMS_FILTER'

// NOT NECESSARY configurationLoad: { asyncResult: true },

// checkedFilter: {},
// stagingFilter: {},
// invalidFilter: {},
// loadingDocuments: {},

// NOT IMPLEMENTED  importDocument: {},
// NOT NECESSARY addDocumentAndOpen: {},
// NOT IMPLEMENTED addAttachment: { asyncResult: true },
// NOT IMPLEMENTED addAttachmentRef: { asyncResult: true },
// NOT IMPLEMENTED deleteAttachmentRef: { asyncResult: true },
// NOT IMPLEMENTED cancelImport: {},

// NOT IMPLEMENTED sesChange: {},
